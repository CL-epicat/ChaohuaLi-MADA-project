...
# Fit models with and without covariates and evaluate performance using 10-fold cross-validation. 
  Model 1: log_PCP as the only predictor;
  
  Model 2: log_PCP and all covariates.
...
## Fit Model 1: log_PCP as the only predictor
```{r}
...
## Fit Model 2: log_PCP and all covariates.
```{r}
...
``` 
  ...
  Model 1: log_PCP as the only predictor - RMSE=358
  
  Model 2: log_PCP and all covariates    - RMSE=292

Model 2 with covariates has much better performance. 


## Fit Model 1 and Model 2 with the train set, and create predicted vs observed values plot

Fit Model 1 and Model 2 with train set.
```{r}
...
combined_predictions <- bind_rows(pred1, pred2)
``` 
...
For Model 2, the data points are generally scattered around the diagonal line and the predicted values are closer to the observed values than the data points for Model 1. 
Thus, Model 2 with all covariates has better performance than Model 1, and is selected as the final model to describe the relationship between mortality rates and PCP supply.
...
# Apply final model to the test set
